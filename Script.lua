...
;(function()
	...
	-- ============================================================
	--                AIMBOT SYSTEM V2.0
	--         Sistema de Aimbot Indetectable y Completo
	-- ============================================================
	
	-- Variables del sistema de aimbot
	local AimbotPlayers = game:GetService("Players")
	local AimbotRunService = game:GetService("RunService")
	local AimbotUserInputService = game:GetService("UserInputService")
	local AimbotWorkspace = game:GetService("Workspace")
	local AimbotCamera = AimbotWorkspace.CurrentCamera
	local AimbotLP = AimbotPlayers.LocalPlayer
	...
	local function GetLocalHRP()
		local char = AimbotLP.Character
		return char and char:FindFirstChild("HumanoidRootPart")
	end
	
	local function IsTeamMate(player)
		if not AimbotConfig.TeamCheck then return false end
		return player.Team == AimbotLP.Team and player.Team ~= nil
	end
	
	local function IsVisible(targetPart)
		if not AimbotConfig.VisibleCheck then return true end
		local localHRP = GetLocalHRP()
		if not localHRP then return false end
		local ray = Ray.new(AimbotCamera.CFrame.Position, (targetPart.Position - AimbotCamera.CFrame.Position).Unit * 2000)
		local ignoreList = {AimbotLP.Character, CurrentTarget and CurrentTarget.Parent}
		for _, v in pairs(AimbotConfig.IgnoreList) do
			table.insert(ignoreList, v)
		end
		local hit, position = AimbotWorkspace:FindPartOnRayWithIgnoreList(ray, ignoreList)
		return hit and hit:IsDescendantOf(targetPart.Parent)
	end
	
	local function GetDistanceFromCenter(position)
		local screenPos, onScreen = AimbotCamera:WorldToViewportPoint(position)
		if not onScreen then return math.huge end
		local centerX, centerY = AimbotCamera.ViewportSize.X / 2, AimbotCamera.ViewportSize.Y / 2
		local distX = screenPos.X - centerX
		local distY = screenPos.Y - centerY
		return math.sqrt(distX * distX + distY * distY)
	end
	
	local function IsInFOV(position)
		if not AimbotConfig.FOVEnabled then return true end
		return GetDistanceFromCenter(position) <= AimbotConfig.FOVRadius
	end
	
	local function GetBestTarget()
		local bestTarget = nil
		local shortestDistance = math.huge
		for _, player in pairs(AimbotPlayers:GetPlayers()) do
			if player ~= AimbotLP and player.Character then
				...
				if humanoid and humanoid.Health > 0 and targetPart then
					...
					local distance = GetDistanceFromCenter(targetPart.Position)
	...
	local function PredictPosition(targetPart)
		...
		local prediction = targetPart.Position + (velocity * AimbotConfig.PredictionAmount)
		return prediction
	end
	
	local function GetHumanizedOffset()
		if not AimbotConfig.HumanizeMovement then
	...
	local function GetDynamicSmoothing()
		if not AimbotConfig.RandomizeSmoothing then
			return AimbotConfig.SmoothAmount
		end
		local variation = math.random(80, 120) / 100
		return AimbotConfig.SmoothAmount * variation
	end
	...
	local function CreateFOVCircle()
		...
		FOVCircle.Filled = AimbotConfig.FOVFilled
	...
	local function UpdateFOVCircle()
		...
		FOVCircle.Position = Vector2.new(AimbotCamera.ViewportSize.X / 2, AimbotCamera.ViewportSize.Y / 2)
		FOVCircle.Radius = AimbotConfig.FOVRadius
		FOVCircle.Color = AimbotConfig.FOVColor
		FOVCircle.Transparency = AimbotConfig.FOVTransparency
		FOVCircle.Filled = AimbotConfig.FOVFilled
		FOVCircle.Visible = AimbotConfig.FOVEnabled and AimbotConfig.Enabled
	end
	...
	local function AimAt(targetPart)
		...
		predictedPos = predictedPos + GetHumanizedOffset()
		local targetCFrame = CFrame.new(AimbotCamera.CFrame.Position, predictedPos)
		if AimbotConfig.Smoothing then
			local smoothing = GetDynamicSmoothing()
			AimbotCamera.CFrame = AimbotCamera.CFrame:Lerp(targetCFrame, smoothing)
		else
			AimbotCamera.CFrame = targetCFrame
		end
	end
	...
	local function AimbotLoop()
		...
		if CurrentTarget and CurrentTarget.Parent then
			...
			if currentTime - TargetLockStartTime > AimbotConfig.MaxLockTime then
				CurrentTarget = nil
				TargetLockStartTime = currentTime
				return
			end
			if IsInFOV(CurrentTarget.Position) and IsVisible(CurrentTarget) then
				AimAt(CurrentTarget)
				return
			else
				if AimbotConfig.StickyAim then
					if currentTime - LastLockTime < AimbotConfig.StickyDuration then
						AimAt(CurrentTarget)
						return
					end
				end
	...
	local function TriggerBot()
		...
		local distance = GetDistanceFromCenter(CurrentTarget.Position)
		if distance < 30 then
			task.wait(AimbotConfig.TriggerDelay)
			mouse1press()
			task.wait(0.05)
			mouse1release()
		end
	...
	local function SetupInputs()
		AimbotUserInputService.InputBegan:Connect(function(input, gameProcessed)
			if gameProcessed then return end
			if input.UserInputType == AimbotConfig.AimbotKey or input.KeyCode == AimbotConfig.AimbotKey then
				IsAiming = true
			end
		...
		AimbotUserInputService.InputEnded:Connect(function(input, gameProcessed)
			if input.UserInputType == AimbotConfig.AimbotKey or input.KeyCode == AimbotConfig.AimbotKey then
				IsAiming = false
				CurrentTarget = nil
			end
	...
	local function StartAimbot()
		...
		CreateFOVCircle()
		AimbotConnection = AimbotRunService.RenderStepped:Connect(function()
			UpdateFOVCircle()
			AimbotLoop()
		end)
	end
	...
	Tabs.Aimbot:CreateSection("🎯 Configuración Principal")
	
	Tabs.Aimbot:CreateToggle({
		Name = "Activar Aimbot",
		Description = "Mantén CLICK DERECHO para apuntar",
		CurrentValue = false,
		Callback = function(Value)
			if Value then
				StartAimbot()
	...
	Tabs.Aimbot:CreateToggle({
		Name = "Verificar Equipo",
		Description = "No apuntar a aliados",
		CurrentValue = true,
		Callback = function(Value)
	...
	Tabs.Aimbot:CreateToggle({
		Name = "Solo Objetivos Visibles",
		Description = "No apuntar a través de paredes",
		CurrentValue = true,
		Callback = function(Value)
	...
	Tabs.Aimbot:CreateSection("🎯 Sistema de Apuntado")
	
	Tabs.Aimbot:CreateDropdown({
		Name = "Parte Objetivo",
		Description = "Parte del cuerpo a apuntar",
	...
	Tabs.Aimbot:CreateToggle({
		Name = "Suavizado (Anti-Detección)",
		Description = "Movimiento suave y realista",
		CurrentValue = true,
		Callback = function(Value)
	...
	Tabs.Aimbot:CreateSlider({
		Name = "Cantidad de Suavizado",
		Description = "Más alto = más suave (0.08-0.15 recomendado)",
		Range = {1, 50},
		Increment = 1,
		CurrentValue = 8,
		Callback = function(Value)
			AimbotConfig.SmoothAmount = Value / 100
		end
	})
	
	Tabs.Aimbot:CreateToggle({
		Name = "Predicción de Movimiento",
		Description = "Predice hacia donde se moverá el objetivo",
		CurrentValue = true,
		Callback = function(Value)
	...
	Tabs.Aimbot:CreateSlider({
		Name = "Cantidad de Predicción",
		Description = "Ajusta la predicción del movimiento",
		Range = {5, 30},
		Increment = 1,
		CurrentValue = 13,
		Callback = function(Value)
			AimbotConfig.PredictionAmount = Value / 100
		end
	})
	
	Tabs.Aimbot:CreateSection("⭕ Campo de Visión (FOV)")
	
	Tabs.Aimbot:CreateToggle({
		Name = "Mostrar FOV",
		Description = "Mostrar círculo de campo de visión",
		CurrentValue = true,
		Callback = function(Value)
			AimbotConfig.FOVEnabled = Value
		end
	})
	
	Tabs.Aimbot:CreateSlider({
		Name = "Radio del FOV",
		Description = "Tamaño del círculo FOV",
		Range = {50, 500},
		Increment = 10,
		CurrentValue = 150,
		Callback = function(Value)
	...
	Tabs.Aimbot:CreateColorPicker({
		Name = "Color del FOV",
		Description = "Color del círculo FOV",
		Color = Color3.fromRGB(255, 255, 255),
		Callback = function(Value)
	...
	Tabs.Aimbot:CreateToggle({
		Name = "FOV Relleno",
		Description = "Rellenar el círculo FOV",
		CurrentValue = false,
		Callback = function(Value)
			AimbotConfig.FOVFilled = Value
		end
	})
	
	Tabs.Aimbot:CreateSection("⚡ Características Avanzadas")
	
	Tabs.Aimbot:CreateToggle({
		Name = "Sticky Aim",
		Description = "Mantiene el objetivo por unos segundos",
		CurrentValue = true,
		Callback = function(Value)
			AimbotConfig.StickyAim = Value
		end
	})
	
	Tabs.Aimbot:CreateSlider({
		Name = "Duración Sticky",
		Description = "Tiempo que mantiene el objetivo (segundos)",
		Range = {1, 10},
		Increment = 1,
		CurrentValue = 3,
		Callback = function(Value)
			AimbotConfig.StickyDuration = Value
		end
	})
	
	Tabs.Aimbot:CreateToggle({
		Name = "Trigger Bot",
		Description = "Presiona E para disparar automático (detectable)",
		CurrentValue = false,
		Callback = function(Value)
			AimbotConfig.TriggerBot = Value
		end
	})
	
	Tabs.Aimbot:CreateSlider({
		Name = "Delay del Trigger",
		Description = "Retraso antes de disparar (ms)",
		Range = {5, 50},
		Increment = 5,
		CurrentValue = 10,
		Callback = function(Value)
			AimbotConfig.TriggerDelay = Value / 100
		end
	})
	
	Tabs.Aimbot:CreateSection("🛡️ Anti-Detección")
	
	Tabs.Aimbot:CreateToggle({
		Name = "Suavizado Aleatorio",
		Description = "Varía el suavizado para parecer humano",
		CurrentValue = true,
		Callback = function(Value)
			AimbotConfig.RandomizeSmoothing = Value
		end
	})
	
	Tabs.Aimbot:CreateToggle({
		Name = "Movimiento Humanizado",
		Description = "Añade micro-movimientos aleatorios",
		CurrentValue = true,
		Callback = function(Value)
			AimbotConfig.HumanizeMovement = Value
		end
	})
	
	Tabs.Aimbot:CreateSlider({
		Name = "Tiempo Máx. Lock",
		Description = "Tiempo máximo de lock continuo (segundos)",
		Range = {1, 5},
		Increment = 1,
		CurrentValue = 2,
		Callback = function(Value)
			AimbotConfig.MaxLockTime = Value
		end
	})
	
	Tabs.Aimbot:CreateSection("ℹ️ Información")
	
	Tabs.Aimbot:CreateParagraph({
		Title = "🎮 Controles",
		Text = "• CLICK DERECHO (mantener) - Activar aimbot\n• Tecla E - Trigger Bot\n\n⚠️ Recomendaciones:\n• Suavizado: 0.08-0.15 (indetectable)\n• FOV: 100-150 (moderado)\n• Activa Anti-Detección\n• No uses Trigger Bot en competitivo"
	})
	
	Tabs.Aimbot:CreateButton({
		Name = "🥷 Configuración Sigilosa",
		Description = "Activa la mejor configuración anti-detección",
		Callback = function()
			AimbotConfig.SmoothAmount = 0.12
			AimbotConfig.FOVRadius = 120
			AimbotConfig.RandomizeSmoothing = true
			AimbotConfig.HumanizeMovement = true
			AimbotConfig.MaxLockTime = 2
			AimbotConfig.TriggerBot = false
		end
	})
	
	Tabs.Aimbot:CreateButton({
		Name = "⚡ Configuración Agresiva",
		Description = "Más rápido pero más detectable",
		Callback = function()
			AimbotConfig.SmoothAmount = 0.05
			AimbotConfig.FOVRadius = 200
			AimbotConfig.TriggerBot = true
		end
	})
