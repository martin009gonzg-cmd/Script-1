...
local Compkiller = {
	Version = '2.6',
	Logo = "rbxassetid://125064196202041",
	Windows = {},
	Scale = {
		Window = UDim2.new(0, 485,0, 565),
		Mobile = UDim2.new(0, 350, 0, 300),
		TabOpen = 185,
		TabClose = 85,
	},
...
Compkiller.Colors = {
	Highlight = Color3.fromRGB(90, 110, 160),
	Toggle = Color3.fromRGB(80, 70, 220),
...
function Compkiller:RainbowHighlight(Speed)
	Speed = Speed or 3
	Compkiller.CustomHighlightMode = true;
	Compkiller._RainbowActive = true;
	task.spawn(function()
		local hue = 0
		while Compkiller._RainbowActive do
			hue = (hue + (Speed / 100)) % 1
...
function Compkiller:StopRainbow()
	Compkiller._RainbowActive = false;
end;
...
function Compkiller.new(Config : Window)
	...
	function WindowArgs:DrawTab(TabConfig : TabConfig , Internal)
		...
		function TabArgs:DrawSection(config: Section)
			...
			local IsOpen = not (config.Minimized == true);
...
function Compkiller:Loader(IconId,Duration)
	...
	Loader.BackgroundColor3 = Color3.fromRGB(10, 8, 30)
	...
	Vignette.ImageColor3 = Color3.fromRGB(80, 70, 220)
	...
	local _rainbowActive = true

	-- Rainbow en Icon y Vignette durante toda la animacion de entrada
	task.spawn(function()
		local h = 0
		while _rainbowActive and Icon and Icon.Parent do
			h = (h + 0.012) % 1
			local c = Color3.fromHSV(h, 0.85, 1)
			Icon.ImageColor3 = c
			Vignette.ImageColor3 = c
			task.wait(0.03)
		end
	end)
...
function Compkiller:RainbowHighlight(element, speed)
	speed = speed or 0.04
	task.spawn(function()
		local h = 0
		while element and element.Parent do
			h = (h + speed) % 1
			local rainbow = Color3.fromHSV(h, 0.85, 1)
			if element.BackgroundTransparency < 1 then
				element.BackgroundColor3 = rainbow
			end
			if element:IsA("ImageLabel") or element:IsA("ImageButton") then
				element.ImageColor3 = rainbow
			end
			if element:IsA("TextLabel") or element:IsA("TextButton") or element:IsA("TextBox") then
				element.TextColor3 = rainbow
			end
			task.wait(0.03)
		end
	end)
